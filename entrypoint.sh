...
find ./.next -type f \( -name "*.js" -o -name "*.css" -o -name "*.html" \) -print0 | while read -r -d $'\0' file; do
    ...
    sed -i "s|\"__TIERS_JSON__\"|${TIERS_JSON_SINGLE_LINE}|g" "$file"
    sed -i "s|\"__NEXT_PUBLIC_FOOTER_LINKS__\"|${NEXT_PUBLIC_FOOTER_LINKS_SINGLE_LINE}|g" "$file"
    sed -i "s|\"__NEXT_PUBLIC_NAVBAR_LINKS__\"|${NEXT_PUBLIC_NAVBAR_LINKS_SINGLE_LINE}|g" "$file"
done
