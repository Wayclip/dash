...
const LoginClientComponent = () => {
    ...
    useEffect(() => {
        ...
        if (params.get('2fa_required') === 'true' && params.get('token')) {
            setTwoFAToken(params.get('token')!);
            setShow2FA(true);
        ...
        try {
            ...
            if (isAxiosError(error) && error.response?.data?.error_code === 'EMAIL_NOT_VERIFIED') {
                toast.error(error.response.data.message, {
                    action: { label: 'Resend Email', onClick: () => handleResendVerification(email) },
                    duration: 10000,
                });
            } else {
    ...
    const handle2FALogin = async (e: FormEvent) => {
        ...
        try {
            const response = await axios.post(
                `${config.apiUrl}/auth/2fa/authenticate`,
                { '2fa_token': twoFAToken, code: twoFACode },
                { withCredentials: true },
            );
            if (response.data.success) {
                await refreshUser();
    ...
    const handleResendVerification = async (email: string) => {
        ...
        if (!config?.apiUrl) return;
        try {
            const response = await axios.post(`${config.apiUrl}/auth/resend-verification`, { email });
    ...
    if (isAuthLoading || isConfigLoading || isAuthenticated) {
    ...
    const anyOAuthEnabled = config?.githubAuthEnabled || config?.googleAuthEnabled || config?.discordAuthEnabled;
    const showSeparator = anyOAuthEnabled && config?.emailAuthEnabled;

    return (
        <div className='flex items-center justify-center min-h-screen bg-background/40'>
            <Card className='w-full max-w-md'>
                <CardHeader className='text-center'>
                    <CardTitle className='text-2xl'>Welcome to {config?.appName || 'Wayclip'}</CardTitle>
